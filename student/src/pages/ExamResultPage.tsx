...
import { getExamResults } from '@/services/examsService';
...
export default function ExamResultPage() {
  ...
  const { id } = useParams();
  const { answers, score, totalMarks, questions, isAssignmentMode } = location.state || {};
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-l from-accent/10 to-background border border-border rounded-2xl p-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-2">
            {isAssignmentMode ? 'نتيجة الواجب' : 'نتيجة الامتحان'}
          </h1>
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-foreground">
            {isAssignmentMode ? 'مراجعة الواجب' : 'مراجعة الإجابات'}
          </h2>
        ...
        <div className="flex gap-4 justify-center pb-8">
          <Button 
            variant="outline" 
            onClick={() => navigate(isAssignmentMode ? '/assignments' : '/exams')}
          >
            {isAssignmentMode ? 'العودة للواجبات' : 'العودة للامتحانات'}
          </Button>
          <Button 
            onClick={() => navigate(`/${isAssignmentMode ? 'assignments' : 'exams'}/${id}/take`)}
          >
            {isAssignmentMode ? 'إعادة الواجب' : 'إعادة الامتحان'}
          </Button>
        </div>
      </div>
    </AppShell>
